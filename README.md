...
Repast4Py uses PyTorch in its value layer raster implementation, 
and the default Linux PyTorch wheels on PyPI with GPU support can be very large
(e.g. several gigabytes). For models that do not use value layers or for
non-GPU Linux environments, the CPU-only build can be used. 
...
The same two steps work for a no-mpi installation,
