...
method, instead a state update is sent to a each ghost agent. In that case, the agent's `update` 
...
is the state element of the tuple returned from the agent's `save` method, namely,
