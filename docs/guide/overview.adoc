...
// Image cross ref isn't working -- should work with <<img-network-ghost>> 
// showing up as Figure 1. <<img-network-ghost,Figure 1>> works but link doesn't seem to
Figure 1 illustrates how ghosts are used in a network projection. The top part of the figure shows that agent `A1` is local to process 1 and has
...
of agents, allowing the user to work with only the local agents.

Spatial projections such as a grid or continuous space are stitched together through the use of 
*_buffers_* and ghosts agents. This is illustrated in Figure 2.

[#img-grid-buffer,reftext='{figure-caption} {counter:refnum}.']
.Ghost Agents in a Buffered Area
image::shared_grid_agents_800.png[]

Here in Figure 2, the full 6x8 grid is distributed across 4 process ranks. Each rank is responsible for
its own 3x4 quarter of the global grid. This is illustrated on the left hand side of Figure 2. 
