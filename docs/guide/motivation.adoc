...
If you only need to run smaller scale models on a single process, Repast4Py can be
installed in a single-rank mode that requires neither MPI nor mpi4py; see
<<single_rank_install>>.
