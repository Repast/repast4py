...
pickle(f"checkpoint_{MPI.COMM_WORLD.Get_rank()}.ckp", ckp)    #<6>
...
adding the current rank to the file name to separate it from checkpoints saved on other ranks
...
ckp = unpickle(f"checkpoint_{MPI.COMM_WORLD.Get_rank()}.ckp")   #<3>
...
rank into the checkpoint file name as in the examples above. When restoring each rank
will restore its own checkpoint and then perform a
https://repast.github.io/repast4py.site/apidoc/source/repast4py.context.html#repast4py.context.SharedContext.synchronize[SharedContext.synchronize]
