...
<17> Now that the zombie has moved, we query its new location for any humans that are present. If humans are present, we then use the `infect()` method from our Model class to infect the colocated humans. Note, as the Zombie is only moving 0.25 spaces, it is possible for the grid location after moving to be the same as the one before moving. 
...
All agents must have a restore function that can take a tuple produced by the save method and return an agent. The `restore_agent` function for the zombie and the human agents are the same, and was described in detail in <Saving and Restoring Agents>.   
