...
For the Zombies model, we utilize an additional Python library, known as `Numba`. 

One of the drawbacks of the Python--an _interpreted_ language--is that code runs slower than other languages, specifically, _compiled_ languages such as `C++`. However, 
...
- Numba compiles the function for the given argument types before it will actually run the function 
- However, this function is then cached, and thus, does not need to be compiled again during the runtime
- "Numba reads the Python bytecode for a decorated function and combines this with information about the types of the input arguments to the function. It analyzes and optimizes your code, and finally uses the LLVM compiler library to generate a machine code version of your function, tailored to your CPU capabilities. This compiled version is then used every time your function is called."

==== The Grid Neighbor Finder class

Every agent at every tick much search their neighborhood of grid locations to determine which grid location has the most humans or the least zombies. Because this neighborhood of grid locations is dependent on each agent's current location, the neighborhood must be computed every timestep for every agent. As the grid neighborhood can be represented as a set of arrays of integers, ... 

Before we utilize a Numba XX, we must first must declare the data types of our variables (this step is different than if we were to just keep this as a general Python numerical process as data type checking is only done when the code is run).
