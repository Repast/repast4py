== How to Write a Repast4Py Simulation
...
The Model class encapsulates the simulation and is responsible for initialization. It  schedules events, creates agents and the grid the agents inhabit, and logging. In addition, the scheduled events
...
synchronized across processes. Events are added to the schedule for execution at a particular "tick".
...
is the start tick, and the second is the frequency for repeating. This schedules `Model.step`
...
This is called at the time specified with `schedule_stop`.
...
----
<1> A BoundingBox is used to initialize the size of Repast4Py's cartesian spaces. Its
...
agent colocations (_meets_) to create an aggregate total number of _meets_. Here, the user
creates a _logger_, which is responsible for logging the dataclass field(s)
...
----
<1> MeetLog is the dataclass used by the aggregate reducing logging. As we saw in
...
across processes. The created logger is added to the list of loggers created
in 4.
<7> Creates a `logging.ReducingDataSet` from the list of loggers where `params['meet_log_file]`
...
<3> Reset the `self.meet_log` values back to 0 because we want to log the data per tick, rather than in aggregate.
...
<3> Update `self.meet_log` with each agent's colocation data by calling `count_colocations`
...
<5> Reset the `self.meet_log` values back to 0 because we want to log the data per tick, rather than in aggregate.
...
ensuring that any remaining unwritten data is written to their respective
files.
...
<5> Check if the `Walker` unique id is in the cache. If it is, retrieve it.
