...
# These tests mirror tests/test_space.py but exercise the distributed SharedGrid
# and SharedCSpace classes on a single rank, where the shared space's local bounds
# equal the global bounds and behavior matches the non-distributed Grid /
# ContinuousSpace. They run under both the mock (single-rank) and default
# native-MPI builds when launched single-process.

comm = MPI.COMM_WORLD


def setUpModule():
    # These mirror the non-distributed space tests and assume the shared space's
    # local bounds == global bounds, which only holds on a single rank.
    if comm.Get_size() != 1:
        raise unittest.SkipTest("single-rank test (run without mpirun or with -n 1)")
...
class SharedGridTests(unittest.TestCase):
    ...
    def test_move(self):
        ...
        grid = space.SharedGrid("grid", bounds=box, borders=BorderType.Sticky, occupancy=OccupancyType.Multiple, buffer_size=0, comm=comm)
    ...
    def test_periodic_move(self):
        ...
        box = space.BoundingBox(xmin=0, xextent=20, ymin=0, yextent=25, zmin=-1, zextent=5)
        grid = space.SharedGrid("grid", bounds=box, borders=BorderType.Periodic, occupancy=OccupancyType.Multiple, buffer_size=0, comm=comm)
    ...
    def test_single_occ(self):
        ...
        box = space.BoundingBox(xmin=0, xextent=20, ymin=0, yextent=25, zmin=-1, zextent=5)
        grid = space.SharedGrid("grid", bounds=box, borders=BorderType.Sticky, occupancy=OccupancyType.Single, buffer_size=0, comm=comm)
        self.assertEqual('grid', grid.name)
        ...
        grid = space.SharedGrid("grid", bounds=box, borders=BorderType.Periodic, occupancy=OccupancyType.Single, buffer_size=0, comm=comm)
        self.assertEqual('grid', grid.name)
...
class SharedCSpaceTests(unittest.TestCase):

    def test_move(self):
        a1 = core.Agent(1, 0)

        box = space.BoundingBox(xmin=0, xextent=20, ymin=0, yextent=25, zmin=-1, zextent=5)
        cspace = space.SharedCSpace("cspace", bounds=box, borders=BorderType.Sticky, occupancy=OccupancyType.Multiple, buffer_size=0, comm=comm, tree_threshold=100)
    ...
    def test_periodic_move(self):
        ...
        box = space.BoundingBox(xmin=0, xextent=20, ymin=0, yextent=25, zmin=-1, zextent=5)
        cspace = space.SharedCSpace("cspace", bounds=box, borders=BorderType.Periodic, occupancy=OccupancyType.Multiple, buffer_size=0, comm=comm, tree_threshold=100)
    ...
    def test_single_occ(self):
        ...
        box = space.BoundingBox(xmin=0, xextent=20, ymin=0, yextent=25, zmin=-1, zextent=5)
        cspace = space.SharedCSpace("cspace", bounds=box, borders=BorderType.Sticky, occupancy=OccupancyType.Single, buffer_size=0, comm=comm, tree_threshold=100)
        ...
        cspace = space.SharedCSpace("cspace", bounds=box, borders=BorderType.Periodic, occupancy=OccupancyType.Single, buffer_size=0, comm=comm, tree_threshold=100)
    ...
    def test_within2D(self):
        ...
        cspace = space.SharedCSpace("cspace", bounds=box, borders=BorderType.Sticky, occupancy=OccupancyType.Multiple, buffer_size=0, comm=comm, tree_threshold=10)
